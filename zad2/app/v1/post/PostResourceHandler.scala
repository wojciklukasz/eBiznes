...
class PostResourceHandler @Inject()(
  ...
  def delete(id: String)(implicit mc: MarkerContext): Future[Option[PostResource]] = {
    postRepository.delete(id).map { postDataList =>
      postDataList.map(postData => createPostResource(postData))
    }
  }

  def update(id: String, postInput: PostFormInput)(implicit mc: MarkerContext): Future[Option[PostData]] = {
